...
void shipControl(float deltaTime){
	inputEntry(deltaTime);
	glm::vec3 distanceObject = direction * glm::vec3(15.0f,15.0f,15.0f);

	// Projection matrix : 45� Field of View, 4:3 ratio, display range : 0.1 unit <-> 100 units
	ProjectionMatrix = glm::perspective(glm::radians(FoV), 4.0f / 3.0f, 0.1f, 100.0f);
	// Camera matrix
	ViewMatrix       = glm::lookAt(
								position-distanceObject,           // Camera is here
...
void inputEntry(float deltaTime){
	...
	if (glfwGetKey( window, GLFW_KEY_W ) == GLFW_PRESS){
		...
		posDelta = direction * deltaTime * speed;
	}
	// Move backward
	...
	// Strafe right
		...
		posDelta = right * deltaTime * speed;
	}
	// Strafe left
		...
		posDelta = glm::vec3(0.0f,0.0f,0.0f);
	}
